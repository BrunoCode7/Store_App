...
class GetProductsListUseCaseTest {
    ...
    fun testProcess()  = runBlockingTest {
        ...
        whenever(productRepository.getProducts()).thenReturn(flowOf(listOf(product1,product2,product3)))

        val response = useCase.process(GetProductsListUseCase.Request).first()

        assertEquals(GetProductsListUseCase.Response(listOf(product1,product2,product3)), response)
